...
class AverageMeter(object):
    ...
    def __init__(self):
        self.reset() #在创建对象时自动调用。该方法调用reset()方法，以初始化所有计数器和变量
...
def auc(pred_property_array, one_hot_labels, num_classes):
    """Return an AUROC list of AUROC score of all classes"""
    AUROCs = []
    # pdb.set_trace()
...
def simple_accuracy(preds, labels):
    # 参数preds是一个一维数组，包含了模型对所有样本的预测标签。数组的每个元素是一个整数，表示模型对该样本的预测标签。
    # 参数labels是一个一维数组，包含了所有样本的真实标签。数组的每个元素也是一个整数，表示该样本的真实标签。
    # print(preds)
    # print(labels)
...
def save_model_auc(args, model):
    """To save the model of best auc"""
    model_to_save = model.module if hasattr(model, 'module') else model
    model_checkpoint = os.path.join(args.output_dir, "%s_bestauc_checkpoint.bin" % args.name)
    torch.save(model_to_save.state_dict(), model_checkpoint) # 它使用PyTorch内置的state_dict()函数将模型的权重保存到磁盘上
    logger.info("Saved model checkpoint to [DIR: %s]", args.output_dir)


def save_model_loss(args, model):
    """To save the model of best loss"""
    model_to_save = model.module if hasattr(model, 'module') else model
...
def load_weights(model, weight_path, args):
    # 参数weight_path是一个字符串，表示保存模型权重的文件路径。
...
def setup(args):
    ...
    # print(num_classes)
    if args.model_type == "ViT-B_16": # TRUE
...
def train(args, model):
    ...
    while True:
        ...
        for step, batch in enumerate(epoch_iterator):
            # for i in batch[1]:
            #     print(i.shape)
            batch = tuple(t.to(args.device) for t in batch) #t(0) = x, t(1) = y
            x, y = batch
            logits = model(x)
